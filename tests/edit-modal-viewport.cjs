...
let keyboardCSS="",safeCSS="";
win.getComputedStyle=()=>({getPropertyValue:name=>name==="--keyboard-height"?keyboardCSS:name==="--safe-area-inset-bottom"?safeCSS:"",paddingTop:"0px",paddingBottom:"0px",borderTopWidth:"0px",borderBottomWidth:"0px",marginTop:"0px",marginBottom:"0px"});
...
class Modal { constructor(app){this.app=app;this.containerEl=new Element("modal-container");this.bgEl=this.containerEl.createDiv("modal-bg");this.modalEl=this.containerEl.createDiv("modal");this.modalEl.createDiv("modal-header-button");this.headerEl=this.modalEl.createDiv("modal-header");this.titleEl=this.headerEl.createDiv("modal-title");this.contentEl=this.modalEl.createDiv("modal-content");modal=this;} open(){this.onOpen();} close(){this.onClose();} }
...
(async()=>{
  ...
  for(let iteration=0;iteration<3;iteration++){
    ...
    const wrapper=find(edit.contentEl,e=>e.classes.has('private-server-mobile-edit'));
    assert(wrapper);assert.equal(wrapper.parentElement,edit.contentEl);
    assert.equal(edit.contentEl.parentElement,edit.modalEl);assert.equal(edit.titleEl.parentElement,edit.headerEl);
    assert.equal(input.parentElement,wrapper);assert.equal(footer.parentElement,wrapper);
    assert(find(wrapper,e=>e.text==='Edit message'));
    const notice=find(wrapper,e=>e.text==='Existing images will be kept.');assert.equal(notice.parentElement,wrapper);
  ...
  const diagnostics=moduleAt('EditModalDiagnostics.ts');assert.equal(typeof diagnostics.EDIT_MODAL_DEBUG,"boolean");
  ...
  // Actual reported iPhone inputs, plus contracted-viewport and absent-variable cases.
  const effective=diagnostics.effectiveEditHeight;
  assert.equal(effective(932,932,0,345),587);
  assert.equal(effective(932,587,0,345),587,'keyboard occlusion is not subtracted twice');
  assert.equal(effective(932,932,0,0),932);
  assert.equal(effective(932,587,24,345),563,'offset and bottom boundary share layout coordinates');
  assert.equal(effective(932,500,0,0),500,'missing variable uses visual bounds');
  keyboardCSS='345px';safeCSS='345px';win.innerHeight=932;diagnosticDoc.documentElement.clientHeight=932;
  win.visualViewport.height=932;win.visualViewport.offsetTop=0;
  footer.getBoundingClientRect=()=>({top:556,bottom:600,height:44,left:0,width:150});
  report=diagnostics.editModalGeometry(container,modalEl,content);
  assert.equal(report.keyboardBoundary,587);assert.equal(report.effectiveAvailableHeight,587);
  assert.equal(report.safeAreaVariable,'345px');assert.equal(report.footerWithinKeyboardSafeRegion,false);assert.equal(report.overflowPixels,13);
  assert.equal(report.footerWithinVisibleViewport,true,'visual viewport alone missed this bug');
  keyboardCSS='';report=diagnostics.editModalGeometry(container,modalEl,content);assert.equal(report.effectiveAvailableHeight,932);
  // Values above test reporting arithmetic only, not WebKit placement.
  const css=fs.readFileSync('styles.css','utf8').split('/* Intersect visual bounds')[1];
  ...
  console.log('PASS: Edit prefill/save/cancel/failure, mobile visible-height/offset updates, actual native header/content plus compact owned wrapper structure, native keyboard/visual intersection, repeated listener cleanup, window fallback, desktop unchanged. No visual/iOS keyboard claims.');
