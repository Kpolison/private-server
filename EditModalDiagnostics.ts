...
export const EDIT_MODAL_DEBUG = false;
...
export function editModalGeometry(container: HTMLElement, modal: HTMLElement, content: HTMLElement) {
  ...
  const describe = (element: HTMLElement | null | undefined) => {
    ...
    return {
      ...
      css: style ? { position: style.position, top: style.top, transform: style.transform,
        ...
        gap: style.gap, boxSizing: style.boxSizing } : null,
    };
  };
  const bottom = (viewport?.offsetTop ?? 0) + (viewport?.height ?? win?.innerHeight ?? 0);
  const footerBottom = footer?.getBoundingClientRect().bottom;
  return {
    innerHeight: win?.innerHeight, clientHeight: doc.documentElement.clientHeight,
    scrollY: win?.scrollY,
    visualViewport: viewport ? { height: viewport.height, offsetTop: viewport.offsetTop,
      pageTop: viewport.pageTop, scale: viewport.scale } : null,
    availableBottom: bottom, footerBottom,
    footerWithinVisibleViewport: footerBottom === undefined ? null : footerBottom <= bottom,
